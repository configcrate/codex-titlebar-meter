...
The compact overlay stays on the right side of the title bar so it does not cover the app menus. It follows the Codex UI language (`localeOverride`, with the system language as fallback) and reads quota snapshots from the local `codex app-server`. Click the small color swatch to change the accent. Install per user with `install.ps1`, or run the executable as a portable app.
